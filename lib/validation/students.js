...
function validateListStudentsQuery(query) {
  ...
  if (query?.exam_date_filter !== undefined) {
    const allowedExam = [
      'today',
      'tomorrow',
      'this_week',
    ...
    if (!isString(query.exam_date_filter) || !allowedExam.includes(String(query.exam_date_filter))) {
      errors.exam_date_filter = 'exam_date_filter قيمة غير مدعومة';
    } else {
      out.exam_date_filter = String(query.exam_date_filter);
    }
  }

  if (query?.activity_filter !== undefined) {
    const allowedActivity = ['inactive_gt_30_days'];
    if (!isString(query.activity_filter) || !allowedActivity.includes(String(query.activity_filter))) {
      errors.activity_filter = 'activity_filter قيمة غير مدعومة';
    } else {
      out.activity_filter = String(query.activity_filter);
    }
  }
